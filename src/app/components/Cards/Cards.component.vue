...
<script setup lang="ts">
  ...
  const { items, filteredItems, pagination, filterInfo }: any = storeToRefs( store );
...
<template>
  <main class="main-container">
    <div class="d-flex fd-col align-cent inner-container">
      ...
      <div class="mt-5 pl-5 pr-5" v-else-if="filteredItems?.length === 0 && items?.length !== 0">
        <h2>{{ filterInfo }}</h2>
        <img src="../../../assets/png/notfound-icon.png" alt="not found">
      </div>
      <div class="mt-5 pl-5 pr-5" v-else>
        <h2>Cargando...</h2>
      </div>
    </div>
  </main>
